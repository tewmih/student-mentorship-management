  ...
  return (
    <Card className="bg-white border-[#e7e7e7]">
      <CardHeader>
        ...
        <p className="text-2xl font-bold text-[#464255]">{typeof totalValue === 'number' ? totalValue.toLocaleString() : totalValue}</p>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          {data.map((dept, index) => (
            <div key={index} className="flex items-center gap-4">
              <div className="w-20 text-sm text-[#737373] text-right">{dept.name}</div>
              <div className="flex-1 relative">
                <div className="h-6 bg-[#f3f2f7] rounded overflow-hidden">
                  <div
                    className="h-full rounded"
                    style={{
                      width: `${Math.min((dept.value / calculatedMaxValue) * 100, 100)}%`,
